require "mvclib/version"
require "mvclib/utils"
require "mvclib/routing"
require "mvclib/helpers"
require "mvclib/controller"
require "active_record"
require 'active_support/inflector'
#项目通过在config.ru文件中Myapp < mvclib::Application的方式继承.
  ...
  class Application
    def call(env)
      ...
      if rack_app
        ...
        Rack::Directory.new(MVCSample.root).call(env)
      else
        [404, {},['not found']]
      end
    end
    ...
    def initialize
      #设置或获取项目根目录的绝对地址
      mvclib.root File.dirname(caller[0].split(":")[0])
      #获取数据库连接配置, 建立连接
      databse_config = File.join(MVCSample.root, "config", "database.yml")
      if File.exist? databse_config
        ActiveRecord::Base.establish_connection YAML::load(File.open(databse_config))
      end
      MVCSample.app_obj self
    end
